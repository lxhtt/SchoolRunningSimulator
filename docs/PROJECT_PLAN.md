...
| **P1 内核** | S1 已实现：可行性求解器、A2 步频策略与夹取解析；S2 已实现：运行计划与限 jerk 速度规划；S3 已实现：真值采样、状态机与步事件；S4 已实现：路线投影与可复现 GPS 噪声；S5 已实现：CSV/JSON/GPX 导出与最小 CLI；S6 待实现（校准） | **模型是否生理可信**——默认值仍全部未校准；真实校准路径见 P1 设计文档 | S1–S5 云端 run `35996689052`：83/83 JVM 测试通过；后续按切片验收 |
